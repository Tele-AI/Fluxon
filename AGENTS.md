...
- Runtime indexes: when events and cleanup already carry a parent key, prefer one `parent -> child -> state` authority structure; prohibit full-table scans on hot paths, keep ABA generations within the owning index, and justify every secondary index. See [Developer - 8 - Data Structure and Index Design Guidelines](<fluxon_doc_en/dev_doc/Developer - 8 - Data Structure and Index Design Guidelines.md>) and [开发者 - 8 - 数据结构与索引设计规约](<fluxon_doc_cn/dev_doc/开发者 - 8 - 数据结构与索引设计规约.md>).
