...
- `commands` 是交给 `ci_runner` 顺序执行的命令列表；
...
### 9.1 `ci_rust` 与 top-attention wrapper 的边界

当前需要区分两层事实：

- `fluxon_test_stack/ci_test_list.yaml` 仍是 suite 示例契约。示例里的 `ci_rust` 命令保留为内联 `cargo run ... kv_test ...`。
- `fluxon_test_stack/ci_2_virt_node.py` 在生成 GitHub Actions / 本地双逻辑节点 CI 用 suite 时，会把 `ci_rust` 的 `kv_test` 命令重写为：
...
- suite 示例继续表达 canonical Rust 测试入口；
- 实际 CI 路径仍由单次 `ci_2_virt_node.py` 调用统一拥有，并在生成 suite 时显式经过 top-attention 索引层；
- GitHub Actions 里定义的 `ci_rust` 轮次选择会先进入 `ci_2_virt_node.py` 参数，再流入生成后的 `profile.runtime.ci.command_tokens`，最后由 `_bin_kvtest.py` 转发给 Rust `kv_test`；
- `_bin_kvtest.py` 继续保持 thin wrapper，只负责把参数转发到 `cargo run --bin kv_test`。

因此，GitHub Actions 现在覆盖的是“由单一 `ci_2_virt_node.py` 入口启动，并通过 top-attention 索引入口执行 `kv_test`”这条真实 CI 路径，而不是拆成第二个独立 CI 入口或复制一份新的 Rust 命令字符串。
