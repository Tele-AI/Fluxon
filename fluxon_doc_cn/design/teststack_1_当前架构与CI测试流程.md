...
然后这些导出会被写入 `ci_prepare_env.sh`，由远端 `ci_runner.sh` 在真正执行命令前 source。该文件只拥有环境变量导出，不修改调用方的 shell option。
...
- 在 source 后恢复 runner 自己的 shell error mode，保持 `errexit` 关闭
...
`ci_runner.sh` 是 workload 退出码的唯一持久化 owner。每条命令都在 `errexit` 关闭状态下执行，随后立即读取 `$?`、写入 `exit_code.txt`、清除 `inflight_attempt.txt`，最后进入 hold。prepare 文件或被 source 的其他文件不能接管这套 shell error mode。
