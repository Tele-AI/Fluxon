...
- `_bin_kvtest.py`: forwards to the Rust `kv_test` binary command. `ci_test_list.yaml` now exposes this wrapper as the formal `ci_top_attention_bin_kvtest` scene, and workload config flows through profile command tokens.
